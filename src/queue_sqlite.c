...
const char create_queue[] = "CREATE TABLE IF NOT EXISTS QUEUE ("
		...
		"CONSTRAINT QUEUE_PK PRIMARY KEY (FILE))";
...
void queue_add_operation(queue_operation *op) {
	int ret;
	time_t date;
	struct tm *date_tm;
...
void _delete_file(const char *file) {
	int ret;
...
int queue_get_operation_for_file(const char *file2, queue_operation *operation) {
	int ret;
	const unsigned char *file;
	const unsigned char *timestamp;
	int op;
